...
from utils.visualize_trajectory import load_transforms, normalize_poses, visualize_trajectory
...
def draw_json_3d_from_json(json_path, vis_path, title):
    try:
        c2ws, vfovs = load_transforms(Path(json_path), default_vfov=60.0)
        c2ws = normalize_poses(c2ws, mode="first_frame")
        ...
        print(f"[WARN] 3D trajectory visualization failed for {json_path}: {e}")
...
def process_data(opt, output_dir, name, image_path, text_path=None, depth_path=None):
    ...
    if image_path is not None:
        base_prefix = image_path.replace("_rgb.png", "")
    elif text_path is not None:
        base_prefix = text_path.replace("_caption.json", "")
    elif depth_path is not None:
        base_prefix = depth_path.replace("_depth.npy", "")
    ...
    for i in range(opt.test_repeat):
        ...
        def pose_normalize(camera_pose, pred_pose_path):
            ...
            for idx, row in enumerate(camera_numpy):
                ...
                if row.shape[0] >= 18:
                    f_x_i, f_y_i, c_x_i, c_y_i, w_i, h_i = row[-6:].tolist()
                    frame_data.update(
                        {
                            "w": w_i,
                            "h": h_i,
        ...
        def save_results(output_dir, name, camera_pose):
            ...
            if text_path is not None and os.path.exists(text_path):
                shutil.copy(text_path, new_caption_path)
            elif base_prefix is not None and os.path.exists(base_prefix + "_caption.json"):
                shutil.copy(base_prefix + "_caption.json", new_caption_path)

            gt_pose_path = None
            if base_prefix is not None:
                candidate = base_prefix + "_transforms_cleaning.json"
                if os.path.exists(candidate):
                    gt_pose_path = candidate
            ...
            return pred_pose_path, (new_pose_path if gt_pose_path is not None else None)

        pred_pose_path, ref_pose_path = save_results(output_dir, name, camera_pose)

        # Always use the Blender-free 3D trajectory visualizer.
        draw_json_3d_from_json(
            pred_pose_path,
            os.path.join(output_dir, f"{name}_traj.png"),
            title=f"{name} pred",
        )

        if ref_pose_path is not None and os.path.exists(ref_pose_path):
            draw_json_3d_from_json(
                ref_pose_path,
                os.path.join(output_dir, f"{name}_traj_ref.png"),
                title=f"{name} ref",
            )
...
if opt.cond_mode == 'text': 
    ...
    if os.path.isdir(opt.test_path):
        ...
        if len(image_paths) > 0:
            ...
            caption_paths = glob.glob(os.path.join(opt.test_path, "*/*_caption.json"))
            print("Number of captions:", len(caption_paths))
            for caption_path in sorted(caption_paths):
                name = 'test/' + caption_path.split('/')[-2] + '/' + caption_path.split('/')[-1][:-13]
                process_data(opt, output_dir, name, image_path=None, text_path=caption_path)
elif opt.cond_mode == 'depth+image+text': 
