...
from core.utils import destandardize_translation, project_world_origin_to_screen
...
class LMM(nn.Module):
    def __init__(self, opt: Options):
        ...
        def _load_sd_pipe():
            return StableDiffusionPipeline.from_pretrained(opt.sd_model_path)
    ...
    def predict_anchor(self, cond_embeds):
        if cond_embeds is None:
            raise ValueError('use_anchor_head=True requires conditioning tokens.')
        return self.anchor_head(cond_embeds.mean(dim=1))
    ...
    def forward(self, data, step_ratio=1, compute_loss=True):
        ...
        if compute_loss and self.opt.use_anchor_head:
            ...
            loss = loss + self.opt.anchor_loss_weight * loss_anchor
            results['loss_anchor'] = loss_anchor

            if self.opt.anchor_screen_loss_weight > 0:
                if self.opt.camera_translation_norm == 'dataset_p99':
                    anchor_world = destandardize_translation(anchor_pred_float, self.opt, anchor=True)
                else:
                    anchor_scale = self.opt.camera_global_scale if self.opt.camera_norm_mode == 'origin_global' else 1.0
                    anchor_world = anchor_pred_float * float(anchor_scale)
                anchor_rotation = data['anchor_rotation'].to(device=anchor_pred.device, dtype=torch.float32)
                anchor_intrinsics = data['anchor_intrinsics'].to(device=anchor_pred.device, dtype=torch.float32)
                pred_c2w = torch.cat([anchor_rotation, anchor_world[:, :, None]], dim=2)
                pred_screen = project_world_origin_to_screen(pred_c2w, anchor_intrinsics)
                target_screen = data['anchor_screen'].to(device=anchor_pred.device, dtype=torch.float32)
                valid = data['anchor_screen_valid'].to(device=anchor_pred.device)
                valid = valid & torch.isfinite(pred_screen).all(dim=1) & torch.isfinite(target_screen).all(dim=1)
                if valid.any():
                    clip = float(self.opt.anchor_screen_loss_clip)
                    pred_screen = pred_screen[valid].clamp(-clip, clip)
                    target_screen = target_screen[valid].clamp(-clip, clip)
                    loss_anchor_screen = F.smooth_l1_loss(pred_screen, target_screen)
                else:
                    loss_anchor_screen = anchor_pred_float.sum() * 0.0
                loss = loss + self.opt.anchor_screen_loss_weight * loss_anchor_screen
    ...
    def generate(
            ...
            if return_anchor:
                return {
                    'tokens': all_tokens,
                    'anchor_t0': None if anchor_pred is None else anchor_pred.detach().float().cpu(),
                }
            return all_tokens
