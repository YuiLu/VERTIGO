...
def _stats_tensor(values, ref):
    return torch.tensor(values, device=ref.device, dtype=ref.dtype)


def standardize_translation(translation, opt: Options, anchor=False):
    mean_values = opt.camera_anchor_mean if anchor else opt.camera_translation_mean
    scale_values = opt.camera_anchor_scale if anchor else opt.camera_translation_scale
    mean = _stats_tensor(mean_values, translation)
    scale = _stats_tensor(scale_values, translation).clamp_min(1e-6)
    return (translation - mean) / scale


def destandardize_translation(translation, opt: Options, anchor=False):
    mean_values = opt.camera_anchor_mean if anchor else opt.camera_translation_mean
    scale_values = opt.camera_anchor_scale if anchor else opt.camera_translation_scale
    mean = _stats_tensor(mean_values, translation)
    scale = _stats_tensor(scale_values, translation).clamp_min(1e-6)
    return translation * scale + mean


def project_world_origin_to_screen(c2w, intrinsics, eps=1e-6):
    R = c2w[..., :3, :3]
    t = c2w[..., :3, 3]
    origin_cam = torch.matmul(R.transpose(-1, -2), -t.unsqueeze(-1)).squeeze(-1)
    z = origin_cam[..., 2]
    ...
    cx = intrinsics[..., 2]
    cy = intrinsics[..., 3]
    w = intrinsics[..., 4].clamp_min(eps)
    h = intrinsics[..., 5].clamp_min(eps)

    u = (fx * origin_cam[..., 0] / z_safe + cx) / w
    v = (fy * origin_cam[..., 1] / z_safe + cy) / h
